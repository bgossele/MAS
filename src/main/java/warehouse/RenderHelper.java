...
class RenderHelper {
	...
	void drawLine(Point p1, Point p2) {
		gc.get().drawLine(vp.get().toCoordX(p1.x), vp.get().toCoordY(p1.y),
	...
	void drawCurve(Point p1, Point p2, Point control) {
		...
		path.quadTo(vp.get().toCoordX(control.x), vp.get().toCoordY(control.y),
				vp.get().toCoordX(p2.x), vp.get().toCoordY(p2.y));
	...
	void drawCircle(Point p, double radius) {
		gc.get().drawOval(vp.get().toCoordX(p.x - radius),
				vp.get().toCoordY(p.y - radius), vp.get().scale(radius * 2),
				vp.get().scale(radius * 2));
	}

	void fillCircle(Point p, double radius) {
		gc.get().fillOval(vp.get().toCoordX(p.x - radius),
				vp.get().toCoordY(p.y - radius), vp.get().scale(radius * 2),
				vp.get().scale(radius * 2));
	}

	void fillCircle(Point p, int radiusInPixels) {
		gc.get().fillOval(vp.get().toCoordX(p.x) - radiusInPixels,
				vp.get().toCoordY(p.y) - radiusInPixels, radiusInPixels * 2,
				radiusInPixels * 2);
	}

	void drawCircle(Point p, int radius) {
		gc.get().drawOval(vp.get().toCoordX(p.x) - radius,
				vp.get().toCoordY(p.y) - radius, radius * 2, radius * 2);
	}
	...
	void drawArrow(Point from, Point to, int width, int height) {
		final double w = invScale(vp.get(), width);
		final double h = invScale(vp.get(), height);
	...
	double invScale(ViewPort vp, int val) {
		return 1d / vp.scale * val;
	}
	...
	void drawString(String string, Point pos, boolean isTransparent,
		...
		gc.get().drawString(string, vp.get().toCoordX(pos.x) + xOffset,
				vp.get().toCoordY(pos.y) + yOffset, isTransparent);
	}
}
